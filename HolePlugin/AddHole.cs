...
namespace HolePlugin
{
    ...
    public class AddHole : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            if (view3D == null)
            {
                TaskDialog.Show("Ошибка", "Не найден вид 3D");
                return Result.Cancelled;
            }
            ...
            foreach (Duct d in ducts)
            {
                ...
                List<ReferenceWithContext> untersections = referenceIntersector.Find(point, direction)
                    .Where(x => x.Proximity <= curve.Length)
                    .Distinct(new ReferenceWintContextElementEqualityComparer())
                    .ToList();
                foreach (ReferenceWithContext refer in untersections)
                {
        ...
        public class ReferenceWintContextElementEqualityComparer : IEqualityComparer<ReferenceWithContext>
        {
